...
const TOMBSTONE_ID = "33333333-3333-4333-8333-333333333333";
...
const publishedConversation = [
  ...
  adminComment({
    ...
    is_author: 1,
    root_display_name: "讀者甲",
    root_status: "approved",
    reply_to_display_name: "讀者甲",
    reply_to_status: "approved",
  }),
  adminComment({
    id: TOMBSTONE_ID,
    parent_id: ROOT_ID,
    ...
    root_display_name: "讀者甲",
    root_status: "approved",
    reply_to_display_name: "HUIKAI",
    reply_to_status: "approved",
    reply_to_is_author: 1,
  }),
];
...
const articleRows = [
  {
    ...
    comment_count: 3,
    thread_count: 1,
    latest_at: "2026-09-10T00:03:00.000Z",
    page_path: "/posts/first-hackathon/",
  },
  {
    article_key: UNKNOWN_ARTICLE,
    comment_count: 1,
    thread_count: 1,
    latest_at: "2026-09-09T00:00:00.000Z",
    page_path: null,
  },
];
...
try {
  for (const [name, width, height] of [["desktop", 1440, 1000], ["mobile", 390, 844]]) {
    ...
    await page.route("**/api/comments/v1/admin/**", async route => {
      ...
      if (request.method() === "GET" && url.pathname.endsWith("/admin/articles")) {
        ...
        return route.fulfill({
          ...
          body: JSON.stringify({ ok: true, status: "approved", articles: rows, total: articleRows.length, limit, offset, hasMore: offset + rows.length < articleRows.length }),
        });
      }
      ...
      if (request.method() === "GET" && url.pathname.endsWith("/admin/comments")) {
        ...
        return route.fulfill({
          status: 200,
          contentType: "application/json",
          body: JSON.stringify({ ok: true, status, articleKey: articleKey || null, comments: pageRows, total: rows.length, limit, offset, hasMore: offset + pageRows.length < rows.length }),
        });
      }

      if (request.method() === "GET" && url.pathname.endsWith("/admin/pending")) {
        return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ ok: true, comments: pendingRows }) });
      }
      if (request.method() === "POST" && /\/admin\/comments\/[0-9a-f-]+\/(approve|hide|restore|delete)$/i.test(url.pathname)) {
        return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ ok: true, deleted: url.pathname.endsWith("/delete") ? "hard" : undefined }) });
      }
      if (request.method() === "POST" && url.pathname.endsWith("/admin/replies")) {
        return route.fulfill({ status: 201, contentType: "application/json", body: JSON.stringify({ ok: true, id: "55555555-5555-4555-8555-555555555555", status: "approved" }) });
      }
    ...
    const head = await page.evaluate(() => ({
      ...
      manifestRows: (() => {
        try { return JSON.parse(document.querySelector("[data-comments-admin-article-manifest]")?.textContent || "[]").length; }
        catch { return -1; }
      })(),
