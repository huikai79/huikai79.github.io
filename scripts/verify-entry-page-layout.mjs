...
function assertProjects(localeLabel, state, desktop) {
  ...
  if (desktop) {
    ...
    if (!state.cards.length) {
      fail(`${localeLabel}/projects: project grid contains no long-term project cards`);
    } else if (!(state.grid.width > state.cards[0].width * 2.7)) {
      fail(`${localeLabel}/projects: project cards appear stretched instead of retaining the desktop multi-column grid contract`);
    }
